...
5. Synthesize verdicts, cross-repo pattern detection, and the consolidated report on the session model
   (Fable 5) -- judgment and synthesis are never delegated to a Sonnet executor. This conductor-class
   stage IS bound by the ≤10/wave, ≤20/turn session-model cap.
...
- Exceed the fan-out budget (≤10 agents/wave, ≤20/turn) for session-model (Fable) dispatches without
