# Security audit briefing (full text)
...
work order. Owned by `skills/audit-pipeline-security/SKILL.md`.
...
1. Security assessment checklist (for each, document Pass / Fail / N-A with evidence):
   ...
   - [ ] SLSA build level assessed (L0-L3 -- artifact attestations alone = Build Level 2;
     attestations + reusable workflows = Level 3)
...
- CRITICAL: exploitable security bug (script injection, unpinned action with known CVE, fork
  code with full secrets)
...
Proceed with your standard workflow (reference files first -- especially security-hardening.md,
then read all CI config, then produce the audit).
```
